  ...
  return (
    <div>
        <form onSubmit={handleCreate} action="">
            <input ref={titleRef} className='border' type="text" />
            <input ref={descRef} className='border' type="text" />
        ...
        <div className='flex p-5 gap-3 flex-wrap'>
            {
                data?.map((item)=> (
                    <div key={item.id} className='w-80 p-3 shadow-md'>
                        ...
                        <p>{item.desc}</p>
                        <button onClick={() => handleDelete(item.id)} className='bg-red-300'>Delete</button>
                        <button onClick={()=> handleEdit(item)} className='bg-green-300'>Edit</button>
                    </div>
                ))
            }
        </div>
